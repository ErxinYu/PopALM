...
class PPOTrainer(BaseTrainer):
    ...
    def step(
        ...
        values = torch.tensor(values.detach().cpu().numpy().tolist(),
                              dtype=torch.float32).to(self.current_device)
        mini_batch_dict = {
            "queries": queries,
            "responses": responses,
    ...
    def prepare_model_inputs_old(self, queries: torch.Tensor, responses: torch.Tensor):
        if self.is_encoder_decoder:
            ...
            input_data = self.data_collator(
                [{"input_ids": ids, "attention_mask": torch.ones_like(ids).bool()} for ids in input_ids]
            ).to(self.current_device)
    ...
    def prepare_model_inputs(self, queries: torch.Tensor, responses: torch.Tensor):
        ...
        inputs_data = {"attention_mask": torch.stack(batch_attention_mask).to(self.current_device),
                  "position_ids": torch.stack(batch_position_ids).to(self.current_device),
                  "input_ids": torch.stack(batch_input_ids).to(self.current_device),
                  "labels": torch.stack(batch_labels).to(self.current_device),
                  }
        return inputs_data
    ...
    def record_step_stats(self, kl_coef: float, **data):
        ...
        """
        # print("data[logprobs]",data["logprobs"].size())
        # print("data[ref_logprobs]",data["ref_logprobs"].size())
        mask = data.pop("masks")
        # print("mask",mask.size())
        # a = (data["logprobs"] - data["ref_logprobs"])
        # print("a",a.size())
        # b = a * mask
        # print("b",b.size())
    ...
    def save_prefix(self,output_dir: str) -> None:
        ...
        self.accelerator.unwrap_model(self.model).save_pretrained(output_dir, state_dict=filtered_state_dict)
    ...
    def ChatGLM_generate(self, tokenizer, inputs, gen_kwargs={}):
        ...
        outputs = self.accelerator.unwrap_model(self.model).generate(**inputs, **gen_kwargs)
        sequences_scores = self.accelerator.unwrap_model(self.model).pretrained_model.compute_transition_scores(outputs.sequences, outputs.scores, normalize_logits=True)
