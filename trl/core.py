...
def logprobs_from_logits(logits, labels):
    ...
    logpy = torch.gather(logp, 2, labels.unsqueeze(2)) 
    return logpy
...
def print_trainable_parameters(model, des=""):
    """
    Prints the number of trainable parameters in the model.
    """
    trainable_params = 0
    all_param = 0
    print("print_trainable_parameters of ",des)
    index = 0
    for name, param in model.named_parameters():
        all_param += param.numel()
        # print("param:\n",name,param.size())
        # if index == 0:
        #     print("param_no_grad_example:\n",name, param, param.dtype)
        #     index = 1
        if param.requires_grad:
            trainable_params += param.numel()
            print("param:",name, "\n", param, param.dtype)
    print(
        f"trainable params: {trainable_params} || all params: {all_param} || trainable%: {trainable_params / all_param}"
    )
def print_dataset_example(tokenizer,example):
    print("input_ids",example["input_ids"])
    print("inputs", tokenizer.decode(example["input_ids"]))
    print("label_ids", example["labels"])
    print("labels", tokenizer.decode(example["labels"]))
    
def respond_to_batch_new(model, queries, txt_len=32, top_k=0, top_p=1.0):
    """Sample text from language model."""
    input_ids = queries
    ...
    for i in range(txt_len):
        # Get Logits
        # print("torch.cat([start_ids, end_ids]", torch.cat([start_ids, end_ids], dim=-1))
        print("")
        outputs = model(input_ids)
        next_token_logits = outputs[0][:, -1, :]
        ...
        input_ids = torch.cat([input_ids, next_token.unsqueeze(-1)], dim=-1)
        # EOS
        # response_text = tokenizer.decode(130004)
        # # print("response_text:", response_text, tokenizer.decode(130005))
        # if next_token.detach().cpu().numpy()[0] == tokenizer.eos_token_id:
        #     return start_ids
        #print("input_ids",input_ids[0])
        #print("input_ids[:, -txt_len:]",input_ids[:, -txt_len:][0])
        #print("batch[response] = tokenizer.batch_decode(response_tensors)", tokenizer.batch_decode(input_ids[0][:, -txt_len:]))
    return input_ids[:, -txt_len:]
...
def compute_rouge(generated_texts,reference_texts):
    ...
    for i in range(len(generated_texts)):
        ...
        golden_response = [reference_texts[i]]
        metric_results = metric.compute(predictions=response, references=golden_response, use_stemmer=True)
        rouge_score = metric_results["rougeLsum"].mid.fmeasure
        rough_rewards.append(rouge_score)
    return rough_rewards