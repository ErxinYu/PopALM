...
class ValueHead(nn.Module):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.summary = nn.Linear(hidden_size, 1) #4096
    ...
    def forward(self, hidden_states):
        ...
        # print("output_before",output.size())
        output = self.summary(output)
        # print("output_after",output.size())
        return output
...
class AutoModelForCausalLMWithValueHead(PreTrainedModelWrapper):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # for key,vales in base_model_output.items():
        #     print("key",vales.size())
        # print("base_model_output.hidden_states",len(base_model_output.hidden_states))
        last_hidden_state = base_model_output.hidden_states[-1] #[131,batch,4096]
        lm_logits = base_model_output.logits #[batch,131,词表数]
        loss = base_model_output.loss
        # print("lm_logits", lm_logits.size())
        if last_hidden_state.device != self.v_head.summary.weight.device:
            last_hidden_state = last_hidden_state.to(self.v_head.summary.weight.device)
        # print("last_hidden_state",last_hidden_state.size())
        value = self.v_head(last_hidden_state).squeeze(-1) #[131,batch]
