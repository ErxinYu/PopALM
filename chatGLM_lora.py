...
from trl.trainer.trainer_seq2seq import Seq2SeqTrainer, Trainer
...
class ModifiedTrainer(Seq2SeqTrainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        loss = output.loss
        # print("output",output,output["logits"].dtype)
...
def main():
    ...
    config = ChatGLMConfig.from_pretrained(model_args.model_name_or_path,trust_remote_code=True)
    ...
    tokenizer = ChatGLMTokenizer.from_pretrained(model_args.model_name_or_path,trust_remote_code=True)
    model = ChatGLMForConditionalGeneration.from_pretrained("THUDM/chatglm-6b", config=config,trust_remote_code=True).cuda().half()
    ...
    print("model_args.lora_checkpoint",model_args.lora_checkpoint)
    if model_args.lora_checkpoint != None:
        print("111111")
        peft_path = f"{model_args.lora_checkpoint}/chatglm-lora.pt"
    ...
    if training_args.do_train:
        ...
        trainer._save_lora(training_args.output_dir)
    ...
    if training_args.do_predict:
        logger.info("*** Predict ***")
        with torch.autocast("cuda"):    
            predict_results = trainer.predict(predict_dataset,ignore_keys=None, metric_key_prefix="predict", max_length=160, do_sample=True, top_p=0.7, temperature=0.95, num_return_sequences=data_args.num_return_sequences)
